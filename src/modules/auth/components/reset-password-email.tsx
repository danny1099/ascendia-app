...
import { ToastMessage, useTranslations } from "next-intl";
...
export const FormEmail = () => {
  ...
  const onSubmit = async (values: EmailSchema) => {
    ...
    if (error || data?.error) {
      const message = (data?.error?.code as ToastMessage) || "unknown_error";
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <div className="flex h-auto w-full flex-col gap-3 py-2">
            ...
            />
            <Button type="submit" className="w-full" icon="arrowRight" isLoading={formState.isSubmitting}>
