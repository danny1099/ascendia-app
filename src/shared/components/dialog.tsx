...
export const DialogOverlay = ({ className, ...props }: React.ComponentProps<typeof DialogPrimitive.Overlay>) => {
  return (
    <DialogPrimitive.Overlay
      ...
      className={cn(
        "data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 fixed inset-0 z-50 bg-black/20 backdrop-blur-sm dark:bg-black/50",
        className
      )}
      {...props}
    />
...
export const DialogTitle = ({ className, ...props }: React.ComponentProps<typeof DialogPrimitive.Title>) => {
  return (
    <DialogPrimitive.Title
      ...
      className={cn("text-md text-foreground leading-none font-medium tracking-tight", className)}
      {...props}
    />
  );
};
...
export const DialogDescription = ({
  ...
  return (
    <DialogPrimitive.Description
      ...
      className={cn("text-muted-foreground text-2xs text-pretty", className)}
      {...props}
    />
  );
};
